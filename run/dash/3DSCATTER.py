...
STARTING_DRUG = "Jordan 1 Retro High Off-White University Blue"
DRUG_DESCRIPTION = df.loc[df['name'] == STARTING_DRUG]['retail_price'].iloc[0]
DRUG_IMG = df.loc[df['name'] == STARTING_DRUG]['image'].iloc[0]
...
def display_molecule(hoverData):
    ...
    if row.empty:
        return
    description = row['retail_price'].iloc[0]
    return description
